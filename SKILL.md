...
## Workflow — <!--gen:count:phases-->9<!--/gen--> фаз (1–7, включая 3.5 и 6.5)
...
4. **Поиск** [main `sonnet`/medium; sub-agents per-task: `haiku` для web/api, `sonnet` для academic/long-source, `opus` для heavy reasoning subtask] — 4 шага: (4.0) **Source Dispatch** — прогнать каждый подвопрос через `source_dispatch.md` matrix, заполнить plan.md секцию 12 с per-subquestion primary/secondary/fallback каналами. (4.1) Launch — для medium/deep: суб-агенты `subagent_type=general-purpose` (нужен Write — агент сам пишет `sources/NN.md` и сам скорит) в параллель, каждому свой диапазон номеров источников (`s01-s09`, `s10-s19`, ...) и `model:` параметром из `model_routing.md`; для shallow — главный поток сам. (4.2) Fetch & dedup. (4.3) Save в `sources/NN_slug.md` — пишет сам агент в своём диапазоне, в главный поток возвращаются только компактные index-строки (не полные тексты). Loop: после каждого раунда оркестратор может потратить bounded deviation — включая trajectory-чек промежуточных claims (не плодить поиск на неподтверждённом тезисе) и динамическую ревизию outline (ScaffoldAgent). См. `references/source_dispatch.md`, `references/subagents_v2.md`, `references/workflow.md` → Фаза 4.
5. **Claims-ledger + триангуляция** [заполнение `haiku`/low; механическая проверка `haiku`/low] — из index-строк и claim-кандидатов от sub-агентов главный поток собирает `claims.csv` (claim_id, claim, hypothesis, sources, source_types, status, confidence, primary_source). Триангуляция механическая: ≥3 источника И ≥2 разных типа → `triangulated`. **Primary-first правило:** без хотя бы одного primary-источника confidence не может быть выше `medium`. Loop: если после первого прохода есть строки со status ≠ triangulated — **gap-волна** (`haiku`/low): точечные агенты на конкретную дыру («нужен primary», «нужен 3-й тип источника», «противоречие — найти причину»), максимум 2 круга, иначе строка помечается `data-insufficient` (честный результат, не бесконечный поиск). Это внутренний луп Фазы 5, не новая фаза (та же конвенция что и loop Фазы 4). См. `references/source_scoring.md` и `references/workflow.md` → Фаза 5.
...
├── claims.csv                       # Фаза 5 — claim-ledger: claim_id, hypothesis, sources,
│                                     #   source_types, status, confidence, primary_source
...
- Для fetch+save (Фаза 4.1) используй `general-purpose` с явным диапазоном номеров источников — нужен Write. `Explore` — только для чистой разведки без сохранения файлов (read-only). Фаза 6 red team — тоже `general-purpose` (роли судят, а не ищут).
- Не пропускать gap-волну (часть Фазы 5) если в `claims.csv` есть нетриангулированные строки — иначе confidence отчёта завышен. Максимум 2 круга, иначе честно `data-insufficient`.
- Не давать confidence выше `medium` тезису без primary-источника (primary-first правило, Фаза 5).
