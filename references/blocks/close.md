...
**Вход:** `plan.md` секция 0 (User context — кто спрашивает, зачем, как будет использовать отчёт) + `claims.csv` (ключевые тезисы с их confidence).
...
**Антипаттерн:** Повторение TL;DR другими словами. Z12 должен добавлять то, чего нет в TL;DR — персонализацию под конкретный кейс из `plan.md` секции 0, а не пересказ фактов.
